...
New-AzResourceGroupDeployment -ResourceGroupName $rgName -TemplateFile $pwd/az104-10-vms-template.json -TemplateParameterFile $pwd/az104-10-vms-parameters.json -AsJob


Get-Job | select Name, Command

Write-Host "Deploying resources to 'az104-10-rg0'"
